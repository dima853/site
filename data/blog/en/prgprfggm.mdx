...
_This article may have inaccuracies; study this to get a general understanding of what it is._
...
>
> **Clarification:** This is true for _high-quality_ entropy. Modern CPUs have instructions like RDRAND, but for cryptography, multiple entropy sources (timings, ADC noise) are often combined, which indeed requires time to accumulate a sufficient number of bits.

At the same time, in many algorithms (encryption, authentication) we don't need randomness "in a vacuum" — we need the result to _look_ random to anyone who doesn't have the secret key.
...
**Requirement**: the output must be _computationally_ indistinguishable from a truly random string for anyone who doesn't know the seed. No statistical anomalies, no patterns.
