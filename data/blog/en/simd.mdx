...
_Example:_ 4 chefs cook 4 different dishes.

**DATA Parallelism:**
One boss gives **ONE** command, which everyone executes **SIMULTANEOUSLY** on their own data.
_Example:_ Coach to the team: "EVERYONE squat!" — 20 people squat at once.
...
2.  **Separates "What" from "How"**
    - **ISA defines _WHAT_ the processor can do** (which set of commands is available).
    - **Microarchitecture defines _HOW_ it does it** (the internal design, which can be different for different manufacturers).

    _Example:_ Intel and AMD processors have the _same_ ISA (x86), but _different_ internal design.
