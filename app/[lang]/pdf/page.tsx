...
export default async function PdfLibraryPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const groupedBySuper = superGroups
    .map((group) => {
  ...
  const remainingCategories = Object.entries(categories).filter(
    ([name]) => !assignedCatNames.has(name)
  )
  ...
  return (
    <div className="min-h-screen pt-11">
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
        <h1 className="mb-2 text-4xl font-extrabold text-black dark:text-white">PDF Library</h1>
        <p className="mb-12 text-sm text-black/50 dark:text-white/50">{totalFiles} documents</p>
        ...
        {(remainingCategories.length > 0 || uncategorized.length > 0) && (
          <section className="mb-16">
            <h2 className="mb-4 text-2xl font-bold text-black dark:text-white">Other Documents</h2>
            <p className="mb-6 text-sm text-black/50 dark:text-white/50">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              {uncategorized.length > 0 && (
                <div className="rounded-2xl border border-black/[0.05] bg-white/40 p-5 dark:border-white/10 dark:bg-white/5">
